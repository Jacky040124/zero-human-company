...
export type LinqMessageTemplate =
  | 'OUTREACH_V1'
  | 'MAAS_POLICY_V1'
  | 'NEGOTIATION_PROPOSAL_V1'
...
export type LinqConfig = {
  ...
  recipientAddresses: Readonly<Record<string, string | undefined>>
}
...
export class LinqMessageProvider implements ProviderPort<LinqSendRequest, LinqSendResult> {
  ...
  async execute(request: ProviderRequest<LinqSendRequest>): Promise<ProviderResult<LinqSendResult>> {
    ...
    const { recipient } = request.payload
    if (recipient?.consented !== true || !recipient.rolePlayerId) {
      throw new Error('Linq sends require one explicitly consented role-player recipient')
    }
    const recipientAddress = this.config.recipientAddresses[recipient.rolePlayerId]
    if (!recipientAddress) {
      throw new Error(`Linq role-player ${recipient.rolePlayerId} has no configured allowlisted recipient`)
    }
    const text = renderLinqMessage(request.payload)
    if (!request.idempotencyKey) {
      throw new Error('Linq sends require an idempotency key')
    }
    ...
    return {
      ...
      redacted: {
        ...
        recipientFingerprint: fingerprintLinqAddress(recipientAddress),
        chatId,
        service,
      },
    }
...
export function renderLinqMessage(request: LinqSendRequest): string {
  if (request.template === 'OUTREACH_V1') {
    assertNoLinqTemplateArgs(request)
    return [
      'Hengxin Home pilot discussion',
      '',
  ...
  if (request.template === 'MAAS_POLICY_V1') {
    assertNoLinqTemplateArgs(request)
    return 'Hengxin Home demo: please reply with your requested per-seat price for the dining pilot. This message is part of the consented hackathon policy test.'
  }
  if (request.template === 'NEGOTIATION_PROPOSAL_V1') {
    if (Object.keys(request.args).some((key) => key !== 'proposalPrice') || !Number.isFinite(request.args.proposalPrice)) {
      throw new Error('Linq negotiation proposal requires only a finite proposal price')
    }
    return `Proposed commercial terms: EUR ${request.args.proposalPrice} per seat for the two-container pilot. This proposal remains subject to buyer acceptance and contract review.`
  }
  throw new Error('Linq message template is not supported')
}

function assertNoLinqTemplateArgs(request: LinqSendRequest): void {
  if (Object.keys(request.args).length) {
    throw new Error(`Linq ${request.template} does not accept template arguments`)
  }
}
