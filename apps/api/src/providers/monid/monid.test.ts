...
const completedRun = {
  ...
  output: {
    organizations: [{
      ...
      employees: [{ name: 'Jane Employee', email: 'jane.employee@nine.example' }],
      people: [{ name: 'Pat Buyer', phone: '+49 30 87654321' }],
      username: 'nine-buyer',
      linkedin_url: 'https://linkedin.com/in/nine-buyer',
      metadata: { owner: { profile: { email: 'owner@nine.example', phone: '+1 415 555 0123' } } },
    }],
  },
}
...
describe('Monid discovery', () => {
  it('normalizes only allowlisted organization fields and excludes nested person PII', () => {
    const company = normalizeMonidCompany({
      ...
      country: 'DE',
      short_description: 'Furniture buyer; write jane@example.com or @jane_buyer',
      ...
      employees: [{ name: 'Jane Employee' }],
      people: [{ name: 'Pat Buyer' }],
      username: 'jane_buyer',
      social_profile_url: 'https://linkedin.com/in/jane-buyer',
      owner: { profile: { email: 'owner@acme.example', phone: '+1 415 555 0123' } },
    })
    ...
    expect(company).toMatchObject({
      ...
      country: 'DE',
      description: 'Furniture buyer; write [REDACTED_EMAIL] or [REDACTED_USERNAME]',
      researchOnly: true,
      source: {
        organization_id: 42,
        name: 'Acme Möbel',
        ...
        url: null,
        country: 'DE',
        short_description: 'Furniture buyer; write [REDACTED_EMAIL] or [REDACTED_USERNAME]',
      },
    })
    const persisted = JSON.stringify(company)
    for (const pii of [
      'not-returned@example.com', '12345678', 'Jane Employee', 'Pat Buyer', 'jane_buyer',
      'linkedin.com', 'owner@acme.example', '555 0123', 'extraSignal',
  ...
  it('discovers and inspects the allowlisted Apollo endpoint before one synchronous paid run', async () => {
    ...
    const persisted = JSON.stringify({ data: result.data, redactedResponse: result.redacted })
    for (const pii of [
      'Jane Employee', 'jane.employee@nine.example', 'Pat Buyer', '87654321', 'nine-buyer',
      'linkedin.com', 'owner@nine.example', '555 0123', 'buyer@nine.example', '12345678',
    ]) expect(persisted).not.toContain(pii)
  ...
  it('rejects a cross-origin configured route before attaching credentials', async () => {
    const transport = vi.fn()
    const provider = new MonidDiscoveryProvider({
      baseUrl: 'https://api.monid.example',
      apiKey: 'secret',
      routes: { discover: 'https://attacker.example/discover' },
    }, transport)

    await expect(provider.preflight()).rejects.toThrow('must remain on the MONID_API_BASE_URL origin')
    expect(transport).not.toHaveBeenCalled()
  })

  it('revalidates a dynamic run-status route before a credential-bearing request', async () => {
    const transport = vi.fn()
    ...
    const provider = new MonidDiscoveryProvider({
      ...
      pollIntervalMs: 0,
      routes: {
        runStatus: (runId) => runId === 'preflight' ? '/v1/runs/preflight' : '//attacker.example/runs/7',
      },
    }, transport, vi.fn(async () => {}))

    await expect(provider.execute(request)).rejects.toThrow('must remain on the MONID_API_BASE_URL origin')
