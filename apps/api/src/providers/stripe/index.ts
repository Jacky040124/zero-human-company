  ...
  implements ProviderPort<StripeCheckoutRequest, StripeCheckoutResult>
{
  ...
  async execute(
    ...
    if (session.livemode !== expectedLivemode || !session.id || !session.url || !isStripeCheckoutUrl(session.url)) {
      throw new ProviderOutcomeUnknownError(
        `Stripe returned an incomplete, unsafe, or wrong-mode checkout session; expected ${this.config.mode}`,
        session.id,
      )
    }
